// viz2_negative.js - stacked bar chart of negative experiences teens have on social media
// bars are sorted from most to least common total negative experience

function drawNegativeViz(data) {

  // convert strings to numbers and compute total negative (a lot + a little)
  data.forEach(d => {
    d["a lot"]    = +d["a lot"]
  ...
  // sort so the most common negative experiences appear at the top
  ...
  // dark red to light pink to gray
  const color = d3.scaleOrdinal()
    .domain(["a lot","a little","no"])
    .range(["#c0392b","#e8a090","#d4cfc9"])
  ...
  const layerGroups = svg.selectAll(".layer")
    .data(stacked).join("g")
    .attr("class", d => `layer layer-neg-${d.key.replace(/ /g,"-")}`)
  ...
  window.negativeHighlight = function(mode) {
    if (mode === null) {
